...
function validacionComun(campoValidar, idMensaje, requerido) {
  ...
  if(cadenaVacia.test(campoValidar.value)) {
     ...
     if(requerido) {
        despliegaMensaje(idMensaje, "msjError", "Error: Se requiere un valor");
        enfoca(campoValidar);
        return false;
     }
...
// validaClave()
//
// Valida una clave. Una clave esta formada de 3 letras mayusculas y 4 digitos
//
// Parametros:
//    - campoValidar: Elemento a validar
//    - idMensaje:    id del elemento en que se desplegara el mensaje
...
//    true si ok
//
function validaEnteroMaxDigitos(maxDigitos, campoValidar, idMensaje, requerido) {
  var stat = validacionComun (campoValidar, idMensaje, requerido);
  if (stat != prosigue) return stat;
